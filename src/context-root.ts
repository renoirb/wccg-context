/**
 * @fileoverview Context Root to gather unsatisfied context requests (modifications: typings for addEventListener)
 * @author Google LLC
 * @license BSD-3-Clause
 * @source Copy of @lit/context@1.1.6
 * @see https://github.com/lit/lit/blob/%40lit/context%401.1.6/packages/context/src/lib/context-root.ts#L11-L134
 */

import {Context} from './create-context.ts';
import {ContextCallback, ContextRequestEvent} from './context-request-event.ts';
...
export class ContextRoot {
  ...
  attach(element: HTMLElement): void {
    element.addEventListener('context-request', this.onContextRequest as EventListener);
    element.addEventListener('context-provider', this.onContextProvider as EventListener);
  }
  ...
  detach(element: HTMLElement): void {
    element.removeEventListener('context-request', this.onContextRequest as EventListener);
    element.removeEventListener('context-provider', this.onContextProvider as EventListener);
  }
